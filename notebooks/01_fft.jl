...
# ╔═╡ 7829d3f2-6215-11f1-9ea1-3b809c1e411c
begin
    using GLMakie
    using FFTW
end

# ╔═╡ 090a8d8d-419d-4c65-8904-c744fbee14db
md"""
# Fast Fourier Transforms in Julia
I'm new to Julia, but I want to explore numerical weather prediction (NWP) models with it. To start, let's explore plotting and FFT.
"""

# ╔═╡ ab982f68-c48c-471d-9b59-6ac8b35ae190
md"""
## Imports
As always, start with the packages we need for this notebook.
"""

# ╔═╡ 962f425b-53d3-49c9-a5b1-a3d2cfe022af
md"""
## Define the grid
Here we define the discretized grid we want to build work with.
"""

# ╔═╡ d1c45cac-fcbd-4df8-a63b-b9852795d7d2
begin
    L = 1000.0
    Nx = 256
...
# ╔═╡ 9ede7577-245e-452c-a6e3-c88c7ac2b957
md"""
## Define the function
Now let's define a function on this grid. A simple superposition of two sine waves, one low frequency, another high frequency.
"""

# ╔═╡ 1ff9e839-6310-4326-b650-9fbb32e7f3fe
begin
	n1 = 1
	n2 = 30
	f = sin.(n1 * 2π .* x ./ L) .+ 0.5 .* sin.(n2 * 2π .* x ./ L)
	lines(x, f, 
      axis=(title="sin wave", xlabel="x", ylabel="f"),
      color=:blue)
end

# ╔═╡ 87056544-2a00-4845-a228-f17d13f6b3e0
md"""
## Get FFT sampling frequencies
We'll use this later for plotting.
"""

# ╔═╡ 26a7ad56-e937-4849-8c0d-dc4035547418
k = fftfreq(Nx, Nx/L)

# ╔═╡ a971b1ab-04ad-463b-a5dd-8069e5c3028c
md"""
## Perform FFT of function
Now we FFT the function we defined above.
"""

# ╔═╡ cb6faec6-6e92-48d6-aaf7-ff48cbb5dcb7
f_hat = fft(f)

# ╔═╡ e6421eef-f7e1-4b6d-8ea4-b5a8c5acc219
md"""
## Perform inverse-FFT to recover function
Now let's invert the FFT to recover our original function. We only want the real part of the FFT.
"""

# ╔═╡ 41f67fc2-7175-42fd-bd2e-3142495ed0cd
f_recovered = real(ifft(f_hat))

# ╔═╡ c3c34e54-cacb-4b28-b7c8-1f76b07cea75
md"""
## Inspect IFFT error
This should be the same function as before, within machine precision.
"""

# ╔═╡ 45131d57-8ab5-45ca-b365-003a088f7920
maximum(abs.(f_recovered .- f))

# ╔═╡ e3829f3f-73ec-4d07-885b-bf422e5cfff0
md"""
## Plot the power spectrum
This should be very nearly zero with two peaks corresponding to the wavenumbers specified above, at $n_1/L$ and $n_2/L$
"""

# ╔═╡ 2432d3e3-ccac-402f-83cd-19680fc19803
let
    power = abs.(f_hat) .^ 2
    kpos = k[1:Nx÷2]
    ppos = power[1:Nx÷2]
    fig, ax = lines(kpos, ppos,
          axis=(title="Power spectrum", xlabel="k", ylabel="|f̂|²",
                yscale=log10))
    vlines!(ax, [n1/L, n2/L], color=:red, linestyle=:dash, label="expected peaks")
    axislegend(ax)
    fig
end
