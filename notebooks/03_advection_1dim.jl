...
# ╔═╡ b4607b24-6223-11f1-b239-6bc812820449
begin
	using FFTW
	using GLMakie
end

# ╔═╡ e3da21bf-9b4d-4d8f-8b3f-7eb2f6f9c1d0
md"""
# 1-Dimensional Advection

Now that we've taken derivatives with FFTs, now let's apply this to integrate over the 1D advection equation. The 1D advection equation for some field ψ is

``
\frac{\partial \psi}{\partial t} = -c \frac{\partial \psi}{\partial x},
``

where $c$ is the advection speed. We'll need to integrate forward in time, and we'll do that using the fourth-order Runge-Kutta method.
"""

# ╔═╡ b3e4ac32-c006-4701-be42-d5f9446eecdc
md"""
## Imports
Start by importing everything we need
"""

# ╔═╡ b1652033-64c8-4e2b-8cbd-f9295dac5a56


# ╔═╡ 43179a33-4202-4dd1-a41a-a693717c93e0
md"""
## Define parameters

Define parameters for the grid, time stepping, and advection speed.
"""

# ╔═╡ fe38694c-9cef-47f4-b552-30da14a78514
begin
    c = -10.0
    dt = 0.1
    Nt = 300
...
# ╔═╡ 258c7188-c861-4aa9-a3cb-93b95b187725
md"""
## Build grids
Build the spatial and spectral grids.
"""

# ╔═╡ fd5d36de-3b10-4a72-9f40-308a55281d9d
begin
	x = range(0, L, length=Nx+1)[1:Nx]
	k = 2π .* fftfreq(Nx, Nx/L)
end

# ╔═╡ a583503b-d026-4670-b487-062cad7e8800
md"""
## Define the initial function
Define $f(x,t=0)$, a simple Gaussian.
"""

# ╔═╡ 446df36f-840f-4e13-876d-d7bb9fca12e7
begin
    σ = 50.0
    f0 = exp.(-(x .- L/2).^2 ./ σ^2)
    fig, ax = lines(x, f0, color=:black)
    fig
end

# ╔═╡ 623e9358-f418-4eda-82ff-7ae6ed560a0f
md"""
## Define functions for calculations
Now we define the functions for calculating the spatial derivative, integrating in time with RK4, and calculating the right-hand-side of the advection equation.
"""

# ╔═╡ 15e94e7a-a3f7-4829-ab4e-1bd413b04b7f
begin
    ...
    # Right hand side of the advection equation
    rhs(f, k, c) = -c .* spectral_deriv(f, k)

    # RK4 time integration step
    function rk4_step(f, k, c, dt)
        k1 = rhs(f,            k, c)
        k2 = rhs(f .+ dt/2 .* k1, k, c)
        k3 = rhs(f .+ dt/2 .* k2, k, c)
        k4 = rhs(f .+ dt   .* k3, k, c)
        return f .+ dt/6 .* (k1 .+ 2k2 .+ 2k3 .+ k4)
    end
    
end

# ╔═╡ 61f7d00a-516b-4e65-b368-82b2686c4361
md"""
## Integrate to solve
To solve $\psi(x,t=N_t \Delta t)$, we take the RK4 steps our prescribed number of times ($N_t$) and at our time step interval ($\Delta t$).
"""

# ╔═╡ d73231f3-a176-4070-bbc0-19099097657f
begin
    f = copy(f0)
    t = 0.0
    for i in 1:Nt
...
# ╔═╡ 4610fd76-061a-4572-bfea-310f3a7b994f
md"""
## Plot results
Our speed is $c=-10$ meters per second, so after $N_t=300$ time steps of size $\Delta t = 0.1$ seconds, we should see that our initial Gaussian was advected westward by ``N_t \times c \times \Delta t = -300`` meters.
"""

# ╔═╡ 485e999c-1555-43a4-9e2a-33d8caa8de4c
let
    f_exact = exp.(-(x .- L/2 .- c*t).^2 ./ σ^2)
    fig, ax = lines(x, f0, label="initial", color=:black, linestyle=:dash)
    lines!(ax, x, f, label="numerical", color=:blue)
    lines!(ax, x, f_exact, label="exact", color=:red, linestyle=:dash)
    vlines!(ax, [L/2 .+ c*Nt*dt], color=:green, linestyle=:dot, label="expected translation")
    axislegend(ax)
    fig
end
